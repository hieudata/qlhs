...
if(isset($_POST['save'])){
  ...
  $ngayTao = date("d-m-Y");
    ...
    $query=mysqli_query($conn,"select * from giaovien where email ='$email'");
    ...
    $query=mysqli_query($conn,"INSERT into giaovien(firstName,lastName,email,password,phone,maKhoi,maLop,ngayTao) 
    value('$firstName','$lastName','$email','123','$phone','$maKhoi','$maLop','$ngayTao');");

    if ($query) {
        
        $qu=mysqli_query($conn,"update lop set trangthaiLop='1' where Id ='$Id'");
            if ($qu) {
 ...
 if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "edit")
	{
        ...
        $query=mysqli_query($conn,"select * from giaovien where Id ='$Id'");
    ...
    $query=mysqli_query($conn,"update giaovien set firstName='$firstName', lastName='$lastName',
    email='$email', password='123',phone='$phone', maKhoi='$maKhoi',maLop='$maLop'
    where Id='$Id'");
            if ($query) {
                
                echo "<script type = \"text/javascript\">
                window.location = (\"giaovien.php\")
                </script>"; 
            }
            else
            {
...
//--------------------------------DELETE------------------------------------------------------------------
  ...
  if (isset($_GET['Id']) && isset($_GET['maLop']) && isset($_GET['action']) && $_GET['action'] == "delete")
	{
        $Id= $_GET['Id'];
        $maLop= $_GET['maLop'];

        $query = mysqli_query($conn,"DELETE FROM giaovien WHERE Id='$Id'");

        if ($query == TRUE) {

            $qu=mysqli_query($conn,"update lop set trangthaiLop='0' where Id ='$maLop'");
            if ($qu) {
        ...
        <div class="container-fluid" id="container-wrapper">
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active" aria-current="page">Thêm giáo viên mới</li>
            </ol>
          </div>
            ...
            <div class="col-lg-12">
              ...
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-dark">Thêm giáo viên mới</h6>
                    <?php echo $statusMsg; ?>
                </div>
                <div class="card-body">
                  <form method="post">
                   <div class="form-group row mb-3">
                     ...
                     <div class="form-group row mb-3">
                      ...
                      <input type="text" class="form-control" name="phone" value="<?php echo $row['phone'];?>" id="exampleInputFirstName" >
                        </div>
                    </div>
                    <div class="form-group row mb-3">
                        <div class="col-xl-6">
              ...
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-dark">Tất cả giáo viên</h6>
                </div>
                <div class="table-responsive p-3">
                  ...
                  <?php
                      $query = "SELECT giaovien.Id,khoi.khoiLop,lop.id as maLop, lop.tenLop,giaovien.firstName,
                      giaovien.lastName,giaovien.email,giaovien.phone,giaovien.ngayTao
                      FROM giaovien
                        ...
                        while ($rows = $rs->fetch_assoc())
                          {
                            ...
                            echo"
                              <tr>
                                ...
                                <td>".$rows['tenLop']."</td>
                                <td>".$rows['ngayTao']."</td>
                                <td><a href='?action=delete&Id=".$rows['Id']."&maLop=".$rows['maLop']."'><i class='fas fa-fw fa-trash text-danger'></i></a></td>
                              </tr>";
                          }
                      }
                      else
